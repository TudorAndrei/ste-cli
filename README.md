...
mise use -g github:TudorAndrei/ste-cli@0.3.0  # one version
ste version
```
...
`ste-0.3.0-darwin-arm64.tar.gz` and `ste-0.3.0-linux-x64.tar.gz`. Each
...
`go build -ldflags "-X main.Version=0.3.0"`. A build from the source
